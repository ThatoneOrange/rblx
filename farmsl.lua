...
if queueonteleport then
    queueonteleport("loadstring(game:HttpGet('https://raw.githubusercontent.com/ThatoneOrange/rblx/main/farmsl.lua'))()")
end
...
function autofarm.antiafk()
    Player.Idled:Connect(function()
        VirtualUser:CaptureController()
        VirtualUser:ClickButton2(Vector2.new())
    end)
end
coroutine.wrap(autofarm.antiafk)()
local Char = Player.Character or Player.CharacterAdded:Wait()
local humanoidrootpart = Char:WaitForChild('HumanoidRootPart')
local Humanoid = Char:WaitForChild('Humanoid')
function autofarm.diedchecker()
    Humanoid.Died:Connect(function()
        autofarm.serverhop()
        return
    end)
end
coroutine.wrap(autofarm.diedchecker)()
...
function autofarm.TimeInServer()
    local time = 0
    while wait(1) do
        if time >= 60 then
            autofarm.serverhop()
...
coroutine.wrap(autofarm.TimeInServer)()
...
if C4 then
    ...
    while true do
        ...
        if not pile then autofarm.serverhop() return end
